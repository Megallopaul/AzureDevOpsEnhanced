...
class PrReviewToolWindow(
    ...
    private val showAllOrgPrsButton =
        JButton().apply {
    ...
    private val placeholderPanel =
        JPanel(BorderLayout()).apply {
            add(
                JBLabel("Select a Pull Request to review").apply {
    ...
    fun loadPullRequest(pullRequest: PullRequest) {
        ...
        mainContentPanel.add(
            JBLabel("Loading PR #${pullRequest.pullRequestId}...").apply {
                horizontalAlignment = JBLabel.CENTER
                border = JBUI.Borders.empty(20)
            },
            BorderLayout.CENTER,
        ...
        ApplicationManager.getApplication().executeOnPooledThread {
            try {
                ...
                ApplicationManager.getApplication().invokeLater {
                    mainContentPanel.removeAll()
                    mainContentPanel.add(
                        JBLabel("❌ Failed to load PR: ${e.message}").apply {
                            horizontalAlignment = JBLabel.CENTER
                            border = JBUI.Borders.empty(20)
                        },
                        BorderLayout.CENTER,
     ...
     */
    private fun setupReviewWorkspace(
        pullRequest: PullRequest,
        changes: List<paol0b.azuredevops.model.PullRequestChange>,
    ) {
        mainContentPanel.removeAll()
        ...
        logger.info(
            "Setting up review workspace: PR #${pullRequest.pullRequestId}, externalProject=$externalProjectName, externalRepo=$externalRepositoryId",
        )
        ...
        val leftRightSplitter =
            JBSplitter(false, 0.25f).apply {
                ...
                val topBottomSplitter =
                    JBSplitter(true, 0.7f).apply {
