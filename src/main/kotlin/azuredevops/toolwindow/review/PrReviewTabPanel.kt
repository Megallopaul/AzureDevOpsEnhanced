...
import azuredevops.services.AvatarService
...
class PrReviewTabPanel(
    ...
    private val reviewersContainer =
        JPanel().apply {
            layout = BoxLayout(this, BoxLayout.Y_AXIS)
            background = UIUtil.getPanelBackground()
        }
    private val policyChecksContainer =
        JPanel().apply {
            layout = BoxLayout(this, BoxLayout.Y_AXIS)
            background = UIUtil.getPanelBackground()
        }
    ...
    // File tree filter
    private val fileFilterCombo =
        ComboBox(arrayOf("All Files", "Reviewed Files", "Unreviewed Files")).apply {
            selectedIndex = 0
            preferredSize = Dimension(160, 28)
        }

    // Vote dropdown
    private val voteComboBox =
        ComboBox(
            arrayOf(
                "Select Vote",
                "✅ Approve",
    ...
    private fun createHeaderPanel(): JPanel {
        ...
        // PR Title
        val titleLabel =
            JBLabel("<html><b style='font-size:13px;'>${escapeHtml(pullRequest.title)}</b></html>").apply {
                alignmentX = Component.LEFT_ALIGNMENT
        ...
        branchPanel.add(
            JBLabel(pullRequest.getSourceBranchName()).apply {
        ...
        branchPanel.add(
            JBLabel(pullRequest.getTargetBranchName()).apply {
                icon = AllIcons.Vcs.BranchNode
        ...
        branchPanel.add(
            JBLabel("Changes from").apply {
                foreground = JBColor.GRAY
                font = font.deriveFont(11f)
            },
        )
        val commitCountLabel =
            JBLabel("... commits").apply {
                foreground = JBColor.GRAY
    ...
    private fun createPolicyChecksSection(): JPanel {
        ...
        policyChecksContainer.add(
            JBLabel("Loading checks...").apply {
                foreground = JBColor.GRAY
                font = font.deriveFont(11f)
            },
        )
    ...
    private fun createVoteSection(): JPanel {
        ...
        if (pullRequest.isDraft == true) {
            ...
            section.add(
                JBLabel("Draft — voting disabled").apply {
    ...
    private fun updatePolicyChecksUI(
        ...
        if (policies.isEmpty()) {
            ...
            items +=
                CheckItem(
                    ...
                    if (allPassed) {
                        "<html><b>All checks passed</b></html>"
                    } else {
                        "<html><b>Checks pending or not available.</b><br>Merge status: ${pr.mergeStatus ?: "unknown"}</html>"
                    },
                    allPassed,
                )
        } else {
    ...
    private fun createReviewerRow(reviewer: Reviewer): JComponent {
        ...
        // Avatar
        val avatarIcon =
            avatarService.getAvatar(reviewer.imageUrl, 24) {
    ...
    private fun applyFileFilter() {
        val filterMode =
            when (fileFilterCombo.selectedIndex) {
    ...
    private fun startAutoRefresh() {
        refreshTimer =
            Timer(REFRESH_INTERVAL) { refreshData() }.apply {
                isRepeats = true
                start()
            }
    }
    ...
    private fun findComponentByName(
        container: Container,
        name: String,
    ): Component? {
