...
class CommentsPanel(
    ...
    private val filterComboBox =
        ComboBox(arrayOf("All", "Active", "Resolved", "Unresolved")).apply {
            selectedIndex = 0
    ...
    fun loadComments() {
        ...
        commentsContainer.add(
            JPanel(FlowLayout(FlowLayout.CENTER)).apply {
                isOpaque = false
                add(
                    JBLabel("Loading comments...", AllIcons.General.ContextHelp, SwingConstants.CENTER).apply {
        ...
        if (threads.isEmpty()) {
            ...
            commentsContainer.add(
                JPanel(FlowLayout(FlowLayout.CENTER)).apply {
                    isOpaque = false
                    add(
                        JBLabel("No comments found", AllIcons.General.Balloon, SwingConstants.CENTER).apply {
                            foreground = JBColor.GRAY
                            font = JBFont.h4()
                        },
                    )
        ...
        JPanel(BorderLayout()).apply {
            ...
            val labelPanel =
                JPanel(FlowLayout(FlowLayout.LEFT)).apply {
                    isOpaque = false
                    add(
                        JBLabel(fileName, icon, SwingConstants.LEFT).apply {
                            font = JBFont.label().asBold()
                        },
                    )
                    add(
                        JBLabel(filePath).apply {
                            foreground = JBColor.GRAY
                            font = JBFont.small()
                        },
                    )
    ...
    private fun createThreadPanel(thread: CommentThread): JComponent {
        ...
        val headerPanel =
            JPanel(BorderLayout()).apply {
                ...
                // Right: Line Info
                val lineText =
                    thread.threadContext?.let { context ->
        ...
        comments.forEachIndexed { index, comment ->
            ...
            val date =
                comment.publishedDate?.let { dateStr ->
                    try {
            ...
            val authorPanel =
                JPanel(FlowLayout(FlowLayout.LEFT, 0, 0)).apply {
                    isOpaque = false
                    add(
                        JBLabel(authorName, AllIcons.General.User, SwingConstants.LEFT).apply {
                            font = JBFont.label().asBold()
                        },
                    )
