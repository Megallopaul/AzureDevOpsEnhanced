package azuredevops.toolwindow.create

import azuredevops.model.GitBranch
import azuredevops.model.Identity
import azuredevops.services.AzureDevOpsApiClient
import azuredevops.services.GitRepositoryService
import azuredevops.util.NotificationUtil
...
class CreatePullRequestPanel(
    ...
    private val reviewerEditButton =
        JBLabel("✎").apply {
    ...
    private fun buildDirectionPanel(): JComponent {
        val renderer =
            object : com.intellij.ui.SimpleListCellRenderer<GitBranch>() {
    ...
    private fun buildChangesPanel(): JComponent {
        ...
        val treeScroll =
            JBScrollPane(changesTree).apply {
    ...
    private fun buildTextPanel(): JComponent {
        ...
        val insertCommitsBtn =
            JButton(AllIcons.Actions.Commit).apply {
    ...
    private fun buildMetadataPanel(): JComponent {
        ...
        reviewerEditButton.addMouseListener(
            object : MouseAdapter() {
                override fun mouseClicked(e: MouseEvent) {
    ...
    private fun buildActionsPanel(): JComponent {
        ...
        val createAction =
            object : AbstractAction("Create Pull Request") {
                override fun actionPerformed(e: java.awt.event.ActionEvent?) {
                    doCreatePullRequest(isDraft = false)
                }
            }
        val createDraftAction =
            object : AbstractAction("Create Draft Pull Request") {
                override fun actionPerformed(e: java.awt.event.ActionEvent?) {
    ...
    private fun loadData() {
        ProgressManager.getInstance().run(
            object : Task.Backgroundable(project, "Loading PR data...", true) {
    ...
    private fun onBranchesChanged() {
        ...
        val scope =
            azuredevops.services.AzureDevOpsCoroutineScope
                .getInstance(project)
            ...
            selectedReviewers.forEach { entry ->
                val chip =
                    JPanel(FlowLayout(FlowLayout.LEFT, 2, 0)).apply {
                        ...
                        add(
                            JBLabel(entry.identity.displayName ?: "Unknown").apply {
                                font = JBUI.Fonts.smallFont()
                            },
                        )
                        if (entry.required) {
                            add(
                                JBLabel("required").apply {
    ...
    private fun showReviewerChooser(anchor: Component) {
        ...
        val available =
            availableReviewers.filter { identity ->
    ...
    private fun showReviewerTypePopup(
        anchor: Component,
        identity: Identity,
    ) {
    ...
    private fun doCreatePullRequest(isDraft: Boolean) {
        ...
        ProgressManager.getInstance().run(
            object : Task.Backgroundable(
                project,
                if (isDraft) "Creating draft pull request..." else "Creating pull request...",
                false,
            ) {
                override fun run(indicator: ProgressIndicator) {
