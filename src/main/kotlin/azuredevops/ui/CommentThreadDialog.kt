...
class CommentThreadDialog(
    ...
    private val statusComboBox = javax.swing.JComboBox<azuredevops.model.ThreadStatus>()
        ...
        val statusLabel =
            JLabel().apply {
                when {
                    ...
                    thread.status == azuredevops.model.ThreadStatus.Pending -> {
                        text = "◐ Pending"
    ...
    private fun createReplyPanel(): JPanel {
        ...
        statusComboBox.apply {
            ...
            azuredevops.model.ThreadStatus
                .allValues()
                .filter { it != azuredevops.model.ThreadStatus.Unknown }
                .forEach { addItem(it) }

            // Set the current thread status as default
            val currentStatus = thread.status ?: azuredevops.model.ThreadStatus.Active
            ...
            renderer =
                object : javax.swing.DefaultListCellRenderer() {
                    override fun getListCellRendererComponent(
                        ...
                        if (value is azuredevops.model.ThreadStatus) {
            ...
            addItemListener { event ->
                if (event.stateChange == java.awt.event.ItemEvent.SELECTED) {
                    val selectedStatus = selectedItem as? azuredevops.model.ThreadStatus
    ...
    private fun updateThreadStatus(newStatus: azuredevops.model.ThreadStatus) {
        if (isLoading) return
    ...
    private fun triggerGlobalRefresh() {
        ...
        azuredevops.services.CommentsPollingService
            .getInstance(project)
            .refreshNow()
        ...
        if (toolWindow != null && toolWindow.isVisible) {
            ...
            if (content != null) {
                ...
                if (component is azuredevops.toolwindow.CommentsNavigatorPanel) {
