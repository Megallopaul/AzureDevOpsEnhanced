package azuredevops.ui

import azuredevops.model.CompletionOptions
import azuredevops.model.MergeStrategy
...
class CompletePullRequestDialog(
    ...
    init {
        ...
        mergeStrategyCombo =
            JComboBox<MergeStrategyItem>().apply {
        ...
        deleteSourceBranchCheckbox =
            JBCheckBox("Delete source branch after merge", isUserPRCreator).apply {
        ...
        bypassPolicyCheckbox =
            JBCheckBox("Override branch policies (use with caution)", false).apply {
                addActionListener {
                    bypassReasonField.isEnabled = isSelected
                    bypassReasonField.background =
                        if (isSelected) {
        ...
        transitionWorkItemsCheckbox =
            JBCheckBox("Complete linked work items", true).apply {
    ...
    private fun createOptionsPanel(): JComponent {
        ...
        if (!isUserPRCreator) {
            ...
            panel.add(
                JBLabel(
                    "<html><small style='color: #BB5544;'>⚠ Only ${pullRequest.createdBy?.displayName ?: "the PR creator"} can delete the source branch</small></html>",
                ).apply {
                    alignmentX = JComponent.LEFT_ALIGNMENT
    ...
    private fun createCommitMessagePanel(): JComponent {
        ...
        val scrollPane =
            JBScrollPane(mergeCommitMessageArea).apply {
                border =
        ...
        mergeCommitMessageArea.document.addDocumentListener(
    ...
    private fun createCommentPanel(): JComponent {
        ...
        val scrollPane =
            JBScrollPane(commentArea).apply {
                border =
                    JBUI.Borders.compound(
                        JBUI.Borders.customLine(JBColor.border(), 1),
                        JBUI.Borders.empty(4),
    ...
    fun getComment(): String? = commentArea.text.trim().takeIf { it.isNotBlank() }
