...
class WorkItemBranchService(
    ...
    companion object {
        fun getInstance(project: Project): WorkItemBranchService = project.getService(WorkItemBranchService::class.java)
    }
    ...
    fun createBranchFromWorkItem(
        ...
        ProgressManager.getInstance().run(
            object : Task.Backgroundable(
                project,
                "Creating branch from Work Item #${workItem.id}...",
                true,
            ) {
                override fun run(indicator: ProgressIndicator) {
                    try {
                        ...
                        if (pushToRemote) {
                            ...
                            val remoteName =
                                repository.remotes.find { it.name == "origin" }?.name
    ...
    private fun getBranchPrefix(workItemType: String): String =
        when (workItemType.lowercase()) {
            "bug" -> "bugfix"
            "task" -> "task"
