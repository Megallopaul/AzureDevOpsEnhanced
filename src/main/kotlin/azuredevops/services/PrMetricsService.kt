    ...
    fun compute(pullRequests: List<PullRequest>): PrMetrics {
        ...
        val conflictRate =
            if (pullRequests.isNotEmpty()) {
                pullRequests.count { it.hasConflicts() }.toDouble() / pullRequests.size * 100
            } else {
                0.0
            }
        val autoCompleteRate =
            if (pullRequests.isNotEmpty()) {
                pullRequests.count { it.hasAutoComplete() }.toDouble() / pullRequests.size * 100
            } else {
                0.0
            }
